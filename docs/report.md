# Heliograph

Solana state report · generated 2026-09-15 17:38 UTC · run #148 · baseline active

## What needs your attention

Nothing. Every watched metric is inside its normal band. You can skip the rest of this report.

## Network

The chain looks healthy: 4,532 TPS (2,402 non-vote) at 316.3 ms per slot, epoch 1035 is 43.7% done.

| metric | value |
|---|---|
| Health | ok |
| Slot | 447,308,797 |
| Block height | 425,350,596 |
| Epoch | 1035 (43.7% complete) |
| TPS (all / non-vote) | 4,532.4 / 2,401.7 |
| Slot time | 316.3 ms |
| Node version | 4.3.0-rc.0 |

## Validators

678 validators are voting, 11 are delinquent (0.06% of stake, nothing alarming). It takes 18 validators to control a third of stake.

| metric | value |
|---|---|
| Active / delinquent | 678 / 11 |
| Delinquent stake | 0.06% |
| Total stake | 439,248,639 SOL |
| Nakamoto coefficient | 18 |
| Median commission | 5% |
| Median prioritization fee | 0 µlamports |

## Economics

A quiet day: SOL at $100.12 (-2.6% 24h), $5.78B locked, $2.53B traded on DEXs.

| metric | value |
|---|---|
| SOL price | $100.12 (-2.6% 24h) |
| Market cap | $58.77B |
| TVL | $5.78B |
| DEX volume 24h | $2.53B (+41.3% 1d) |
| Chain fees 24h | $13.58M |
| Real economic value 24h | $5.20M |
| Stablecoin supply | $15.73B |

## Supply

| metric | value |
|---|---|
| Circulating | 587,027,828 SOL |
| Non-circulating | 47,083,556 SOL |
| Total | 634,111,384 SOL |

## Ecosystem

Cluster status: All Systems Operational.

Daily active wallets: 2,464,545 (2026-09-14, via Dune).

Recent agave client releases:

- v4.3.0-rc.1 · 2026-09-11
- v4.4.0-alpha.4 (pre-release) · 2026-09-10
- v4.3.0-rc.0 · 2026-09-04
- v4.4.0-alpha.3 (pre-release) · 2026-09-03
- v4.4.0-alpha.2 (pre-release) · 2026-08-28

Recently accepted SIMDs (upcoming protocol changes):


## Sources

| source | status |
|---|---|
| solana_rpc_network | ok |
| solana_rpc_validators | ok |
| solana_rpc_supply | ok |
| coingecko | ok |
| defillama_tvl | ok |
| defillama_dex | ok |
| defillama_fees | ok |
| defillama_stablecoins | ok |
| defillama_rev | ok |
| solana_statuspage | ok |
| github_agave_releases | ok |
| github_simds | ok |
| dune_active_wallets | ok |

---
*Heliograph reads Solana so you don't have to. The dashboard is plumbing; this page is the product.*
