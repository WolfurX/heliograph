# Heliograph

Solana state report · generated 2026-09-12 18:41 UTC · run #130 · baseline active

## What needs your attention

Nothing. Every watched metric is inside its normal band. You can skip the rest of this report.

## Network

The chain looks healthy: 4,426 TPS (2,306 non-vote) at 318.0 ms per slot, epoch 1033 is 56.49% done.

| metric | value |
|---|---|
| Health | ok |
| Slot | 446,500,016 |
| Block height | 424,542,771 |
| Epoch | 1033 (56.49% complete) |
| TPS (all / non-vote) | 4,425.9 / 2,305.5 |
| Slot time | 318.0 ms |
| Node version | 4.3.0-rc.0 |

## Validators

677 validators are voting, 13 are delinquent (0.41% of stake, nothing alarming). It takes 18 validators to control a third of stake.

| metric | value |
|---|---|
| Active / delinquent | 677 / 13 |
| Delinquent stake | 0.41% |
| Total stake | 436,837,681 SOL |
| Nakamoto coefficient | 18 |
| Median commission | 5% |
| Median prioritization fee | 0 µlamports |

## Economics

A quiet day: SOL at $101.7 (+0.2% 24h), $5.90B locked, $3.18B traded on DEXs.

| metric | value |
|---|---|
| SOL price | $101.7 (+0.2% 24h) |
| Market cap | $59.66B |
| TVL | $5.90B |
| DEX volume 24h | $3.18B (+9.0% 1d) |
| Chain fees 24h | $17.88M |
| Real economic value 24h | $6.99M |
| Stablecoin supply | $16.16B |

## Supply

| metric | value |
|---|---|
| Circulating | 586,632,691 SOL |
| Non-circulating | 47,291,276 SOL |
| Total | 633,923,967 SOL |

## Ecosystem

Cluster status: All Systems Operational.

Daily active wallets: 2,354,315 (2026-09-10, via Dune).

Recent agave client releases:

- v4.3.0-rc.1 (pre-release) · 2026-09-11
- v4.4.0-alpha.4 (pre-release) · 2026-09-10
- v4.3.0-rc.0 · 2026-09-04
- v4.4.0-alpha.3 (pre-release) · 2026-09-03
- v4.4.0-alpha.2 (pre-release) · 2026-08-28

Recently accepted SIMDs (upcoming protocol changes):


## Sources

| source | status |
|---|---|
| solana_rpc_network | ok |
| solana_rpc_validators | ok |
| solana_rpc_supply | ok |
| coingecko | ok |
| defillama_tvl | ok |
| defillama_dex | ok |
| defillama_fees | ok |
| defillama_stablecoins | ok |
| defillama_rev | ok |
| solana_statuspage | ok |
| github_agave_releases | ok |
| github_simds | ok |
| dune_active_wallets | ok |

---
*Heliograph reads Solana so you don't have to. The dashboard is plumbing; this page is the product.*
