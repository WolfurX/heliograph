# Heliograph

Solana state report · generated 2026-09-06 21:39 UTC · run #92 · baseline active

## What needs your attention

Nothing. Every watched metric is inside its normal band. You can skip the rest of this report.

## Network

The chain looks healthy: 3,747 TPS (1,624 non-vote) at 316.9 ms per slot, epoch 1029 is 85.43% done.

| metric | value |
|---|---|
| Health | ok |
| Slot | 444,897,048 |
| Block height | 422,941,361 |
| Epoch | 1029 (85.43% complete) |
| TPS (all / non-vote) | 3,746.8 / 1,623.8 |
| Slot time | 316.9 ms |
| Node version | 4.2.2 |

## Validators

676 validators are voting, 17 are delinquent (0.02% of stake, nothing alarming). It takes 18 validators to control a third of stake.

| metric | value |
|---|---|
| Active / delinquent | 676 / 17 |
| Delinquent stake | 0.02% |
| Total stake | 439,248,820 SOL |
| Nakamoto coefficient | 18 |
| Median commission | 5% |
| Median prioritization fee | 0 µlamports |

## Economics

A quiet day: SOL at $106.12 (+2.2% 24h), $5.92B locked, $1.96B traded on DEXs.

| metric | value |
|---|---|
| SOL price | $106.12 (+2.2% 24h) |
| Market cap | $62.13B |
| TVL | $5.92B |
| DEX volume 24h | $1.96B (+4.2% 1d) |
| Chain fees 24h | $10.48M |
| Real economic value 24h | $4.25M |
| Stablecoin supply | $16.35B |

## Supply

| metric | value |
|---|---|
| Circulating | 585,444,822 SOL |
| Non-circulating | 48,103,875 SOL |
| Total | 633,548,697 SOL |

## Ecosystem

Cluster status: All Systems Operational.

Daily active wallets: 2,025,638 (2026-09-05, via Dune).

Recent agave client releases:

- v4.3.0-rc.0 (pre-release) · 2026-09-04
- v4.4.0-alpha.3 (pre-release) · 2026-09-03
- v4.4.0-alpha.2 (pre-release) · 2026-08-28
- v4.3.0-beta.3 (pre-release) · 2026-08-28
- v4.2.2 · 2026-08-28

Recently accepted SIMDs (upcoming protocol changes):


## Sources

| source | status |
|---|---|
| solana_rpc_network | ok |
| solana_rpc_validators | ok |
| solana_rpc_supply | ok |
| coingecko | ok |
| defillama_tvl | ok |
| defillama_dex | ok |
| defillama_fees | ok |
| defillama_stablecoins | ok |
| defillama_rev | ok |
| solana_statuspage | ok |
| github_agave_releases | ok |
| github_simds | ok |
| dune_active_wallets | ok |

---
*Heliograph reads Solana so you don't have to. The dashboard is plumbing; this page is the product.*
