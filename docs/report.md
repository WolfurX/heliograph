# Heliograph

Solana state report · generated 2026-09-19 17:56 UTC · run #173 · baseline active

## What needs your attention

- 🔵 **TVL spiked to 6,242,573,619 USD, 3.2 standard deviations above its recent norm of 5,868,711,207 USD.**
  - evidence: baseline n=172, mean=5,868,711,206.8, current=6,242,573,619.0, z=+3.22

## Network

The chain looks healthy: 4,654 TPS (2,126 non-vote) at 265.5 ms per slot, epoch 1038 is 15.12% done.

| metric | value |
|---|---|
| Health | ok |
| Slot | 448,481,299 |
| Block height | 426,522,083 |
| Epoch | 1038 (15.12% complete) |
| TPS (all / non-vote) | 4,654.0 / 2,126.4 |
| Slot time | 265.5 ms |
| Node version | 4.3.0-rc.0 |

## Validators

677 validators are voting, 13 are delinquent (0.04% of stake, nothing alarming). It takes 18 validators to control a third of stake.

| metric | value |
|---|---|
| Active / delinquent | 677 / 13 |
| Delinquent stake | 0.04% |
| Total stake | 440,227,371 SOL |
| Nakamoto coefficient | 18 |
| Median commission | 5% |
| Median prioritization fee | 0 µlamports |

## Economics

A quiet day: SOL at $111.44 (-0.9% 24h), $6.24B locked, $3.54B traded on DEXs.

| metric | value |
|---|---|
| SOL price | $111.44 (-0.9% 24h) |
| Market cap | $65.45B |
| TVL | $6.24B |
| DEX volume 24h | $3.54B (+36.4% 1d) |
| Chain fees 24h | $17.46M |
| Real economic value 24h | $6.17M |
| Stablecoin supply | $16.51B |

## Supply

| metric | value |
|---|---|
| Circulating | 587,367,785 SOL |
| Non-circulating | 47,008,472 SOL |
| Total | 634,376,256 SOL |

## Ecosystem

Cluster status: All Systems Operational.

Daily active wallets: 3,035,062 (2026-09-17, via Dune).

Recent agave client releases:

- v4.4.0-alpha.5 (pre-release) · 2026-09-18
- v4.3.0 (pre-release) · 2026-09-18
- v4.3.0-rc.1 · 2026-09-11
- v4.4.0-alpha.4 (pre-release) · 2026-09-10
- v4.3.0-rc.0 · 2026-09-04

Recently accepted SIMDs (upcoming protocol changes):


## Sources

| source | status |
|---|---|
| solana_rpc_network | ok |
| solana_rpc_validators | ok |
| solana_rpc_supply | ok |
| coingecko | ok |
| defillama_tvl | ok |
| defillama_dex | ok |
| defillama_fees | ok |
| defillama_stablecoins | ok |
| defillama_rev | ok |
| solana_statuspage | ok |
| github_agave_releases | ok |
| github_simds | ok |
| dune_active_wallets | ok |

---
*Heliograph reads Solana so you don't have to. The dashboard is plumbing; this page is the product.*
